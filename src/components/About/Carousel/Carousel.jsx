...
function Carousel() {
  const [activeIndex, setActiveIndex] = useState(); // Corrected useState syntax
  ...
  useEffect(() => {
    setActiveText(aboutTextMobile[activeIndex]); 
  }, [activeIndex]); 

  const nextSlide = () => {
    setActiveIndex((prevIndex) =>
      prevIndex === aboutTextMobile.length - 1 ? 0 : prevIndex + 1
    );
  };

  const prevSlide = () => {
    setActiveIndex((prevIndex) =>
      prevIndex === 0 ? aboutTextMobile.length - 1 : prevIndex - 1
    );
  };
